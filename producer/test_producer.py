#!/usr/bin/env python3
"""
Simple test script for RTSPProducer
Tests basic functionality without requiring actual RTSP stream
"""

import sys
import os
import numpy as np
import cv2
import pytest
from unittest.mock import Mock, patch
from main import RTSPProducer


@pytest.fixture
def producer():
    """Create a RTSPProducer instance for testing"""
    return RTSPProducer(
        rtsp_url="rtsp://test:554/stream",
        broker_url="http://localhost:3090",
        stream_id="test_stream",
        target_fps=30
    )


def test_producer_initialization():
    """Test that producer can be initialized"""
    print("Testing producer initialization...")
    producer = RTSPProducer(
        rtsp_url="rtsp://test:554/stream",
        broker_url="http://localhost:3090",
        stream_id="test_stream",
        target_fps=30
    )

    assert producer.rtsp_url == "rtsp://test:554/stream"
    assert producer.broker_url == "http://localhost:3090"
    assert producer.stream_id == "test_stream"
    assert producer.target_fps == 30
    assert producer.frame_interval == 1.0 / 30
    print("✓ Producer initialization: PASSED")


def test_producer_fixture_initialization(producer):
    """Test that producer fixture is properly initialized"""
    assert producer.rtsp_url == "rtsp://test:554/stream"
    assert producer.broker_url == "http://localhost:3090"
    assert producer.stream_id == "test_stream"
    assert producer.target_fps == 30
    assert producer.frame_interval == 1.0 / 30
    print("✓ Producer fixture initialization: PASSED")


def test_get_websocket_url(producer):
    """Test WebSocket URL generation"""
    expected_url = "ws://localhost:3090/ws/ingest/test_stream"
    actual_url = producer.get_websocket_url()
    assert actual_url == expected_url
    print("✓ WebSocket URL generation: PASSED")


def test_frame_processing(producer):
    """Test frame processing (JPEG encoding) - updated for actual implementation"""
    print("Testing frame processing...")

    # Create a dummy frame (640x480 RGB)
    dummy_frame = np.zeros((480, 640, 3), dtype=np.uint8)
    dummy_frame[:, :] = [128, 128, 128]  # Gray frame

    # Process frame
    frame_data = producer.process_frame(dummy_frame)

    assert frame_data is not None
    assert isinstance(frame_data, bytes)
    assert len(frame_data) > 0

    # Frame data should be a valid JPEG since that's the default format
    # Check for JPEG header (starts with 0xFFD8)
    assert frame_data[:2] == b'\xff\xd8'
    print("✓ Frame processing: PASSED")


def test_cleanup(producer):
    """Test cleanup"""
    print("Testing cleanup...")
    
    # Mock the cap attribute so it can be released
    mock_cap = Mock()
    mock_cap.release = Mock()
    mock_cap.isOpened = Mock(return_value=False)
    producer.cap = mock_cap
    
    producer.cleanup()
    
    # Verify that cap.release() was called if cap exists
    mock_cap.release.assert_called_once()
    
    print("✓ Cleanup: PASSED")


def main():
    """Run all tests - maintained for manual execution"""
    print("=" * 50)
    print("RTSP Producer Test Suite")
    print("=" * 50)
    print()

    try:
        # Test 1: Initialization
        test_producer_initialization()
        print()

        # Create a producer instance for the manual tests
        producer = RTSPProducer(
            rtsp_url="rtsp://test:554/stream",
            broker_url="http://localhost:3090",
            stream_id="test_stream",
            target_fps=30
        )
        
        # Test 2: WebSocket URL generation
        test_get_websocket_url(producer)
        print()

        # Test 3: Frame Processing
        test_frame_processing(producer)
        print()

        # Test 4: Cleanup
        test_cleanup(producer)
        print()

        print("=" * 50)
        print("All tests PASSED! ✓")
        print("=" * 50)
        return 0

    except AssertionError as e:
        print(f"\n✗ Test FAILED: {e}")
        return 1
    except Exception as e:
        print(f"\n✗ Unexpected error: {e}")
        import traceback
        traceback.print_exc()
        return 1


if __name__ == "__main__":
    sys.exit(main())